...
export default  function RegisterOrLoginForm({title,onSubmitMethod, switcher}:{title:string,onSubmitMethod:CallableFunction,switcher:CallableFunction}) {
    ...
    return <>
        <div className="flex relative flex-wrap  bg-gray-100 border-b-2 border-t-4 max-w-xl  active border-amber-700 mt-40 md:mt-28 p-5 my-auto w-11/12 mx-auto shadow-lg flex-col rounded-lg backdrop-blur-3xl  ">
            <h2 className='text-lg font-bold text-center mx-auto flex-1'>{title ==='Login'?'Connexion':"S'enregistrer"} </h2><br />

            <hr />
            <div className="flex flex-1 p-1">
                <form onSubmit={handleSubmit} className='block  w-full' method="post">
                    <div className='flex flex-wrap w-full mt-2'>
                        <div className=" w-full flex flex-wrap">
                            ...
                            <input required={true} minLength={3} aria-invalid={true} value={mail}  onChange={(e)=>setMail(e.target.value)} className='flex flex-auto  outline-1 outline-gray-300  p-2  ' type="email" name="email" id="mail" />
                        </div>
                    ...
                    <div className='flex flex-wrap w-full mt-2'>
                        <div className=" w-full flex flex-wrap">
                            ...
                            <input required={true} minLength={6} value={pwd}  onChange={(e)=>setPwd(e.target.value)} className='flex-1  p-2  outline-1 outline-gray-300  ' type="password" name="pwd" id="pwd"  />
                        </div>
